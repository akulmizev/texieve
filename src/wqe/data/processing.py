...
class PreFilter:
    def __init__(
        ...
        assert (
            scripts_to_keep or langs_to_keep or apply_c4_filter
    ...
    def __call__(
        ...
        if self.scripts_to_keep:
            logger.info(
                f"Filtering documents for accepted scripts: {', '.join(self.scripts_to_keep)}"
            )
        if self.langs_to_keep:
            logger.info(
                f"Filtering documents for accepted languages: {', '.join(self.langs_to_keep)}"
            )
    ...
    def _pre_filter_doc(
        ...
        if urls_to_remove:
            assert (
                "url" in doc
            ), "URLs to remove specified, but document does not have a `url` field."
        ...
        for line in lines:
            ...
            if "scripts" in patterns:
                ...
                if "cleanup" in patterns:
                    line = re.sub(
                        patterns["cleanup"], lambda x: " " if x.group(0) else "", line
                    )
            ...
            if langs_to_keep:
                assert (
                    model is not None
                ), "Language ID model must be specified for language filtering."
        ...
        if warn_percent > 0.0:
            ...
            if removed_chars / article_length > warn_percent:
                logger.warning(
                    f"Removed {removed_chars} characters from article: {doc['id']}"
                )
    ...
    def _make_regex(self):
        ...
        scripts = "".join([rf"\p{{{script}}}" for script in self.scripts_to_keep])
        accepted_characters = r"\p{M}\p{P}\p{S}\p{N}\p{Z}\p{C}"
        script_regex = rf"[{accepted_characters}]*[{scripts}]+[{accepted_characters}]*"
        self.patterns["scripts"] = re.compile(script_regex)
        brackets = r"[\(\[\{][^\p{L}]+[\)\]\}]"
        cleanup_pattern = rf"^(?!.*[{scripts}]).*$|(?<=\S)\s+(?=\.$)|{brackets}"
...
class Deduplicate:
    ...
    def __call__(
        ...
        if not tokenizer and self.min_hash:
            logger.warning(
                "No tokenizer specified. Splitting on whitespace for minhash."
            )

        # Removing existing id column to avoid conflicts
        ...
        lsh = (
            MinHashLSH(threshold=self.jaccard_threshold, num_perm=Deduplicate.N_PERM)
            if self.min_hash
            else None
        )
    ...
    def _deduplicate_doc(
        ...
        if "tokens" not in doc.keys():
            doc["tokens"] = (
                tokenize(doc["text"])
                if not tokenizer
                else tokenizer.tokenize(doc["text"])
            )

        if exact_match:
            doc["hash"] = insecure_hashlib.md5(
                "".join(doc["tokens"]).encode("utf-8")
            ).hexdigest()
...
class Threshold:
    def __init__(self, thresholds: Dict[str, Union[int, float, str]], **kwargs):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        metrics_to_calculate = [
            metric for metric in self.metrics if metric not in dataset.column_names
        ]
        ...
        for metric, threshold in self.thresholds.items():
            ...
            if threshold == "auto":
                self.thresholds[metric] = self._get_auto_threshold(
                    dataset[metric], higher_is_better
                )
                if higher_is_better:
                    n_docs_removed = len(
                        np.where(np.array(dataset[metric]) < self.thresholds[metric])[0]
                    )
                    logger.info(
        ...
        if not keep_columns:
            columns_to_keep = list(
                set(dataset.column_names) - set(self.metrics + ["tokens"])
            )
    ...
    @staticmethod
    def _apply_thresholds(example: Dict[str, Any], thresholds: Dict[str, int]) -> bool:
    ...
    def _get_auto_threshold(
        values: List[Union[int, float]], higher_is_better: bool
    ) -> float:
        if len(values) == 0:
        ...
        sorted_values = (
            np.array(sorted(values)[:n_docs])
            if higher_is_better
            else np.array(sorted(values)[n_docs:])
        )
        ...
        if higher_is_better:
            ...
            threshold = metric_values[
                np.argmax(value_kde(metric_values) - sampled_kde(metric_values))
            ]
        else:
            metric_values = np.linspace(min(sampled_values), max(sorted_values), n_docs)
            threshold = metric_values[
                np.argmin(value_kde(metric_values) - sampled_kde(metric_values))
            ]
...
class Partition:
    ...
    def select_indices(
        ...
        if self.split_method == "mean_cutoff":
            ...
            partition_point = np.searchsorted(
                cumulative_chars, char_budget, side="right"
            )
        ...
        if (higher_is_better and self.quality) or (
            not higher_is_better and not self.quality
        ):
            return partition_2
        else:
            return partition_1
    ...
    def join_partitions(self, dataset) -> List[int]:
        ...
        if self.join_partitions_by == "intersection":
            partition_indices = [
                self.select_indices(dataset, metric) for metric in self.metrics
            ]
            ...
            raise ValueError(
                "Invalid join method. Please specify either 'intersection', 'union' or 'scores'."
            )

    def combine_and_score_metrics(self, dataset: Dataset):
    ...
    def apply_metrics(
        ...
        if "tokens" not in example.keys():
            example["tokens"] = (
                ...
                if not tokenizer
                else tokenizer.tokenize(example["text"])
            )
