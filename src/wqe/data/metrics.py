...
class AlphaChars(BaseMetric):
    ...
    @classmethod
    def calculate(cls, example: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
        example[cls.ANNOTATION_TAG] = sum(
            1 for char in example["text"] if char.isalpha()
        )
        return example
...
class FracNoScriptWords(BaseMetric):
    ...
    @classmethod
    def calculate(cls, example: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
        n_script_words = float(
            sum(
                int(cls.SCRIPT_REGEX.match(word) is not None)
...
class FracUniqueWords(BaseMetric):
    ...
    @classmethod
    def calculate(cls, example: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
        example[cls.ANNOTATION_TAG] = len(set(example["tokens"])) / len(
            example["tokens"]
        )
        return example
...
class TrigramEntropy(BaseMetric):
    ...
    def calculate(cls, example: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
        ...
        trigram_probs = {
            trigram: count / total_trigrams for trigram, count in trigram_counts.items()
        }
...
class LinesEndWithPunctuation(BaseMetric):
    ...
    def calculate(cls, example: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
        ...
        score = sum(
            1 for line in lines if line.strip().endswith(tuple(ALL_PUNCTUATION))
        ) / len(lines)
...
class FracLinesEndEllipsis(BaseMetric):
    ...
    def calculate(cls, example: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
        lines = example["text"].split("\n")
        score = sum(
            1 for line in lines if line.strip().endswith(cls.ELLIPSIS_SYMBOLS)
        ) / len(lines)
        example[cls.ANNOTATION_TAG] = score
        return example
...
class NumWordsPerLine(BaseMetric):
    ...
    def calculate(cls, example: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
        lines = example["text"].split("\n")
        example[cls.ANNOTATION_TAG] = sum(len(tokenize(line)) for line in lines) / len(
            lines
        )
        return example
...
class Perplexity(ModelBasedMetric):
    ...
    def calculate(cls, example: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
        if not cls.metrics:
            raise ValueError(
                "Perplexities must be calculated via `init_metrics` before calling the metric."
            )
        example[cls.ANNOTATION_TAG] = cls.metrics[example["id"]]
        return example
