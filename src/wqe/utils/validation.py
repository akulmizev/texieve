...
from ..data.loader import LangID
    ...
    if task in ["pos", "ner"]:
        if columns:
            ...
            assert any(
                ...
                for feature in ["tags", "ner_tags", "upos"]
        ...
        if columns:
            ...
            assert all(
                feature in dataset["train"].features
                for feature in ["premise", "hypothesis"]
...
def validate_and_format_splits(
    ...
    if task in ["pos", "ner"]:
        if columns:
            if len(columns) == 2:
                ...
                assert (
                    len(columns) == 3
                ), """If text and tag columns have the same name across train, valid and test datasets, \n
            ...
            assert (
                "tokens" in train.features
            ), "Train split must have a `text` column formatted as features."
            assert (
                "tokens" in valid.features
            ), "Validation split must have a `text` column formatted as features."
            assert (
                "tokens" in test.features
            ), "Test split must have a `text` column formatted as features."
            assert any(
                feature in train.features for feature in ["tags", "ner_tags", "upos"]
            ), "Train split must have a `tags`, `ner_tags`, or `upos` column formatted as features."
            assert any(
                feature in valid.features for feature in ["tags", "ner_tags", "upos"]
            ), "Validation split must have a `tags`, `ner_tags`, or `upos` column formatted as features."
            assert any(
                feature in test.features for feature in ["tags", "ner_tags", "upos"]
        ...
        if columns:
            if len(columns) == 3:
                ...
                assert (
                    len(columns) == 3
                ), """If premise, hypothesis and labels columns have the same name across train, valid and test datasets, \n
            ...
            assert all(
                feature in train.features for feature in ["premise", "hypothesis"]
            ), "Train split must have `premise` and `hypothesis` columns formatted as features."
            assert all(
                feature in valid.features for feature in ["premise", "hypothesis"]
            ), "Validation split must have `premise` and `hypothesis` columns formatted as features."
            assert all(
                feature in test.features for feature in ["premise", "hypothesis"]
            ), "Test split must have `premise` and `hypothesis` columns formatted as features."

            assert ("label" in train.features) or (
                "labels" in train.features
            ), "Dataset must have a `label`, or `labels` column formatted as features."
            assert ("label" in valid.features) or (
                "labels" in train.features
            ), "Dataset must have a `label`, or `labels` column formatted as features."
            assert ("label" in test.features) or (
                "labels" in train.features
            ), "Dataset must have a `label`, or `labels` column formatted as features."
        ...
        if columns:
            if len(columns) == 2:
                ...
                assert (
                    len(columns) == 3
                ), """If text and labels columns have the same name across train, valid and test datasets, \n
                                            specify column names as [<text_column_name>, <label_column_name>]. \n
                                            If they are dfferent then specify column names as: \n
                                            [[train column name list], [valid column name list], [test column name list]]."""
                train_columns = columns[0]
            ...
            assert (
                "text" in train.features
            ), "Train split must have a `text` column formatted as features."
            assert (
                "text" in valid.features
            ), "Validation split must have a `text` column formatted as features."
            assert (
                "text" in test.features
            ), "Test split must have a `text` column formatted as features."
            assert ("label" in train.features) or (
                "labels" in train.features
            ), "Dataset must have a `label`, or `labels` column formatted as features."
    ...
    if task in ["pos", "ner"]:
        dataset = dataset.cast_column(
            "tags", ClassLabel(num_classes=len(set(dataset["train"]["tags"])))
        )
    else:
        dataset = dataset.cast_column(
            "labels", ClassLabel(num_classes=len(set(dataset["train"]["labels"])))
        )
    return dataset
