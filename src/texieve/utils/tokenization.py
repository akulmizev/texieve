...
def merge_tokenizers(
    ...
    if not isinstance(
        tokenizer_to_merge.backend_tokenizer.model,
    ...
    if not has_sentencepiece_backend(base_tokenizer) or not has_sentencepiece_backend(
        tokenizer_to_merge
    ):
        logger.warning(
        ...
        if include_special_tokens:
            base_tokenizer.add_special_tokens(
                {
                    k: v
                    for k, v in tokenizer_to_merge.special_tokens_map.items()
        ...
        tokenizer_to_merge_sp.ParseFromString(
            open(tokenizer_to_merge.vocab_file, "rb").read()
        )
...
class SpmConverter:
    ...
    def __init__(self, original_tokenizer: PreTrainedTokenizer):
        if not hasattr(original_tokenizer, "sp_model"):
            raise ValueError(
                "The provided tokenizer does not have a SentencePiece model attached to it."
            )
    ...
    def converted(self) -> Tokenizer:
        ...
        tokens_to_add = {
            ...
            for _id, token, special in [
                (_id, p.piece, p.type == 3)
                for _id, p in enumerate(self.proto.pieces)
                if p.type in [3, 4]
            ]
        }
        tokens_to_add = [
            k for _, k in sorted(tokens_to_add.items(), key=lambda x: x[0])
        ]
