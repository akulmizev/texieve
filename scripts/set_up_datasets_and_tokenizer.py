...
from wqe import MonolingualLoader, HfSentencePieceTokenizer, GOPHER_THRESHOLDS
...
TOKENIZER_CONFIG = TokenizerConfig(model={"type": "unigram"}, vocab_size="auto")


def make_datasets(wiki_id: str) -> None:
    raw_wiki = (
        MonolingualLoader(wiki_id)
        .load_dataset()
        .generate_splits(test_size=0.05, shuffle=True, seed=42)
    )
    raw_wiki.push_to_hub(f"{HUB_PATH}/raw_wiki", wiki_id)

    tokenizer = HfSentencePieceTokenizer.train_from_config(
        raw_wiki["train"]["text"], TOKENIZER_CONFIG
    )
    tokenizer.push_to_hub(f"{HUB_PATH}/raw_wiki.{wiki_id}")

    pre_filtered = MonolingualLoader.from_dataset(raw_wiki["train"], wiki_id)
    pre_filtered = pre_filtered.pre_filter(script_regex=True)
    pre_filtered = pre_filtered.deduplicate(
        exact_match=True, min_hash=True, jaccard_threshold=0.85, n_shingles=3
    )
    pre_filtered.generate_splits(test_size=0.05, shuffle=True, seed=42)
    pre_filtered.push_to_hub(f"{HUB_PATH}/pre_filtered", wiki_id)

    tokenizer = HfSentencePieceTokenizer.train_from_config(
        pre_filtered["train"]["text"], TOKENIZER_CONFIG
    )
    tokenizer.push_to_hub(f"{HUB_PATH}/pre_filtered.{wiki_id}")

    pre_filtered_concat = concatenate_datasets(
        [pre_filtered["train"], pre_filtered["test"]]
    )
    ...
    for metric in METRICS:
        for quality in [True, False]:
            pre_filtered_train = MonolingualLoader.from_dataset(
                pre_filtered_concat, wiki_id
            )
            ...
            partitioned.generate_splits(test_size=0.05, shuffle=True, seed=42)
