...
def get_stats(language, stats_dir):
    ...
    dataset = MonolingualLoader(language)
    ...
    stats_out.write(
        f"{language}\traw\t{stats_dict['current_chars']}\t{stats_dict['current_docs']}\t{1.0}\t{1.0}\n"
    )
    ...
    dataset.pre_filter(
        lang_id=False, deduplicate_min_hash=True, jaccard_threshold=0.85, num_proc=32
    )
    stats_dict = update_stats_dict(stats_dict, dataset.n_chars, dataset.n_docs)
...
def update_stats_dict(stats_dict, n_chars, n_docs):
    ...
    stats_dict["frac_chars_deleted"] = (
        stats_dict["deleted_chars"] / stats_dict["full_chars"]
    )
    stats_dict["frac_docs_deleted"] = (
        stats_dict["deleted_docs"] / stats_dict["full_docs"]
    )
