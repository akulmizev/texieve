...
from wqe import MonolingualLoader, HfTokenizerFromConfig, validate_and_format_dataset
...
def load_dataset(lang: str) -> Dict[str, List[str]]:
    ...
    """
    if lang == "xh":
        wiki = MonolingualLoader(lang).load_dataset().pre_filter(char_cutoff=100)
    else:
        wiki = (
            MonolingualLoader(lang)
            .load_dataset()
            .pre_filter(script_regex=True, char_cutoff=100)
        )

    ner_dataset = validate_and_format_dataset("masakhane/masakhaner2", lang, "ner")[
        "validation"
    ]
...
def calculate_tokenization_stats(
    dataset: Dict[str, List[str]], tokenizer
) -> Dict[str, float]:
    """
...
def main():
    ...
    for lang in LANGS:
        ...
        for config_file in os.listdir(CONFIG_DIR):
            ...
            tokenizer = HfTokenizerFromConfig.train_from_config(
                dataset["wiki"], tokenizer_config
            )
            stats.append(
                {
                    "lang": lang,
